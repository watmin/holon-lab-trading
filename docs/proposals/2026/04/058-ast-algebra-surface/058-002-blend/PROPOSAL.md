...
Blend entered the algebra core as specified by the 2026-04-18 ACCEPTED decision. Two independent f64 scalar weights, binary arity, negative weights allowed, returns `wat::holon::HolonAST`.

- **Core primitive:** `:wat::holon::Blend (a :wat::holon::HolonAST) (b :wat::holon::HolonAST) (w1 :f64) (w2 :f64) -> :wat::holon::HolonAST`
- **Dispatch:** [`wat-rs/src/runtime.rs`](https://github.com/watmin/wat-rs) — `:wat::holon::Blend` arm in `eval_dispatch`; backed by `wat::holon::HolonAST::blend`
...
|---|---|
| `:wat::holon::Subtract x y` | `Blend(x, y, 1.0, -1.0)` |
| `:wat::holon::Amplify x y s` | `Blend(x, y, 1.0, s)` |
| `:wat::holon::Circular v p` | `Blend(cos-basis, sin-basis, cos θ, sin θ)` |
| `:wat::holon::Reject x y` | `Blend(x, y, 1.0, -dot(x,y)/dot(y,y))` |
| `:wat::holon::Project x y` | `Subtract(x, Reject(x, y))` (transitively Blend) |
| `:wat::holon::Log v min max` | via Thermometer; log-transform then gradient |
...
`Blend` enters the algebra core as a new variant with signature `(:wat::holon::Blend a b w1 w2)` — two independent real-valued scalar weights. All five designer questions closed on the arguments below. Designer review in Round 3 may still reopen any of them; this section records our best reading.
...
- **058-018 Circular** — becomes `(:wat::holon::Blend cos-basis sin-basis (cos θ) (sin θ))`; stdlib macro.
- **058-015 Amplify** — becomes `(:wat::holon::Blend x y 1 s)`; stdlib macro.
- **058-019 Subtract** — becomes `(:wat::holon::Blend x y 1 -1)`; stdlib macro (the canonical delta — 058-004 Difference REJECTED).
- **058-020 Flip** — becomes `(:wat::holon::Blend x y 1 -2)`; stdlib macro.
...
1. **`Linear` reclassified as stdlib:**
   ...
   (:wat::core::define (:wat::std::Linear v scale)
     (:wat::holon::Blend (:wat::holon::Atom :wat::std::linear-low)
                         (:wat::holon::Atom :wat::std::linear-high)
