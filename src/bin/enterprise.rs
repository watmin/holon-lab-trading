...
use enterprise::thought_encoder::ThoughtEncoder;
...
struct Args {
    ...
    /// Update cost (configurable, default 0.0).
    #[arg(long, default_value_t = 0.0)]
    update_cost: f64,
}
...
fn build_enterprise(args: &Args) -> (Enterprise, Ctx) {
    ...
    let thought_encoder = ThoughtEncoder::new(&vm);
    let ctx = Ctx::new(thought_encoder, dims, recalib_interval);
    ...
    let exit_observers: Vec<ExitObserver> = EXIT_LENSES
        .iter()
        .map(|lens| ExitObserver::new(lens.clone(), dims, recalib_interval, 0.015, 0.030, 0.045, 0.030))
        .collect();
        ...
        .map(|slot_idx| {
            ...
            Broker::new(
                ...
                vec![
                    ...
                    ScalarAccumulator::new("tp-distance", ScalarEncoding::Log, dims),
                    ScalarAccumulator::new("runner-trail-distance", ScalarEncoding::Log, dims),
                ],
            )
        })
        .collect();
...
fn display_progress(ent: &Enterprise, candle_num: usize, elapsed_ms: f64) {
    ...
    for b in &post.registry {
        for acc in &b.scalar_accums {
            eprintln!("      accum-{}: count={}", acc.name, acc.count);
        }
    }
}
...
fn main() {
    ...
    // Meta table
    {
        ...
        for (k, v) in &[
            ...
            ("update_cost", args.update_cost.to_string()),
