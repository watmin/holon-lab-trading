...
(define (vwap-distance candles)
  ...
  (let ((accum (fold (lambda (acc c)
                       (let ((typical (/ (+ (:high c) (:low c) (:close c)) 3.0))
                             (vol     (:volume c)))
                         (list (+ (first acc) (* typical vol))
                               (+ (second acc) vol))))
                     (list 0.0 0.0) candles))
        (cum-vol-price (first accum))
        (cum-vol       (second accum)))
    (when (> cum-vol 1e-10)
