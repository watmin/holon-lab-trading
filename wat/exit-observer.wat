...
; Cache misses are returned as values — values up, not queues down.
...
;; cache misses from encoding. Values up, not queues down.
...
(define (evaluate-and-compose [exit-obs : ExitObserver]
  ...
  (let* ((results     (map (lambda (ast) (encode (:thought-encoder ctx) ast))
                           exit-fact-asts))
         ((all-vecs all-misses) (unzip results))
         (misses      (flatten all-misses))
         ;; Bundle market thought with all evaluated exit vectors
         (composed    (apply bundle (append (list market-thought) all-vecs))))
    (list composed misses)))
...
;;
;; Returns: (list distances experience)
