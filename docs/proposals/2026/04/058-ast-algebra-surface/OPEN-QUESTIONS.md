...
- **List combinators** — `map-with-index`, `pairwise-map`, `n-wise-split`, `n-wise-map`, `take`, `length`, `range`, `reverse`, `encode-window`. **Decision pending.** These are heavily used inside stdlib macros (Sequential/Chain/Ngram/Analogy expansions). If they live at `:wat/core/` they're language primitives; if at `:wat/std/list/` they're stdlib. Current guess: the small set used INSIDE stdlib macros needs to be at `:wat/core/list/` (because stdlib macros are the stdlib — they can't depend on stdlib for their own expansion). Others users typically want may live at `:wat/std/list/`. Datamancer deliberating.
