...
;; programs-are-thoughts property. Candles arrive in a stream;
;; a collection of describer programs each produce a Thought
;; that describes the candle from their own perspective;
;; we rank the describers by how well their output matches
;; the candle's own thought-encoding.
;;
...
;; Thought. When the hypothesis doesn't apply to this candle,
...
(define (:demo/desc/hammer [c : :demo/market/Candle]) : Thought
...
(define (:demo/desc/strong-bullish [c : :demo/market/Candle]) : Thought
...
(define (:demo/desc/quiet-day [c : :demo/market/Candle]) : Thought
...
;; ------------------------------------------------------------
;; CANDLE ENCODING — the candle as a Thought in the SAME space
;; as the describers' outputs. Used as the reference point we
;; measure describers against.
;; ------------------------------------------------------------

(define (:demo/encode-candle [c : :demo/market/Candle]) : Thought
  (Sequential
...
(define (:demo/alive? [t : Thought]) : Bool
  (not (equal-ast? t (Atom :null))))


;; ------------------------------------------------------------
;; SCORING — how well does this describer's output match the
;; candle's own thought-encoding?
;;
;; Both describer output and candle encoding are Thoughts.
;; `encode` (lowercase) realizes each Thought to a Vector.
;; `cosine` (lowercase) measures similarity. Lazy — the encoding
;; cache serves repeated lookups.
;; ------------------------------------------------------------

(define (:demo/score [describer-output : Thought]
                     [candle-thought   : Thought]) : Scalar
  (if (:demo/alive? describer-output)
      (cosine (encode describer-output)
              (encode candle-thought))
      0.0))
...
(define (:demo/rank [c          : :demo/market/Candle]
                    [describers : (:List :Function)])
                    : (:List (:Tuple :Function :Scalar))
  (let ([ref    (:demo/encode-candle c)]
        [scored (map (lambda ([d : :Function]) : (:Tuple :Function :Scalar)
                       (list d (:demo/score (d c) ref)))
                     describers)]
        [live   (filter (lambda ([pair : (:Tuple :Function :Scalar)]) : Bool
                          (> (second pair) 0.0))
                        scored)])
...
;;                     Returns a hammer Thought with lower-wick thermometer.
...
;;    The candle's thought-encoding captures (open open-val), (high high-val),
...
;;    a Thought describing the pattern. The Thought IS the
;;    description.
;;
;; 2. DESCRIPTIONS ARE COMPARABLE. Because Thoughts encode to
