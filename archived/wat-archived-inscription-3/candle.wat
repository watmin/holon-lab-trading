; candle.wat — the enriched candle. Raw OHLCV in, 100+ computed indicators out.
;
; Depends on: indicator-bank (produced by IndicatorBank.tick).
;
; Produced by IndicatorBank.tick(raw-candle). The post's first act every candle.
; The vocabulary observes the Candle — it is the input to all fact production.

(require primitives)

;; ── Candle — the enriched data ──────────────────────────────────────────
...
(struct candle
  ...
  ;; Rate of Change — preserves sign (no signum split)
  ...
  ;; Regime (pre-computed by IndicatorBank)
  ...
  [variance-ratio : f64]       ; variance at scale N / (N x variance at scale 1)
  ...
  ;; Divergence (pre-computed by IndicatorBank from PELT peaks)
  ...
  ;; Price action (pre-computed by IndicatorBank)
