...
(define (wilder-step! [s : WilderState] [value : f64])
  ...
  ; After: Wilder smooth = (prev x (period - 1) + value) / period.
  (inc! (:count s))
  (let* ((pf (+ (:period s) 0.0)))
    (if (<= (:count s) (:period s))
      (begin
    ...
    (begin
      ...
      (if (< (ring-len (:history s)) 2)
        ...
        (let* ((n   (+ (ring-len (:history s)) 0.0))
               ...
               (sx  (fold + 0.0 (map (lambda (i) (+ i 0.0)) (range 0 (ring-len (:history s))))))
               (sy  (fold + 0.0 vals))
               (sxx (fold + 0.0 (map (lambda (i) (* (+ i 0.0) (+ i 0.0))) (range 0 (ring-len (:history s))))))
...
(define (compute-choppiness [chop-buf : RingBuffer]
  ...
  : f64
  ; CI = 100 x log10(sum(ATR_14) / (highest_high - lowest_low)) / log10(14).
...
;; ════════════════════════════════════════════════════════════════════════
;; Variance ratio — variance at scale N / (N x variance at scale 1)
;; ════════════════════════════════════════════════════════════════════════
...
(define (compute-aroon-up [buf : RingBuffer])
  : f64
  ; Aroon Up = 100 x (period - periods_since_highest) / period.
...
(define (compute-aroon-down [buf : RingBuffer])
  : f64
  ; Aroon Down = 100 x (period - periods_since_lowest) / period.
  (if (< (ring-len buf) 2)
    50.0
    (let* ((n (ring-len buf))
...
(define (tick [bank : IndicatorBank]
  ...
  ; Returns an enriched Candle with raw OHLCV + 70+ computed indicators.
  ...
  (let* (;; ── 1. Extract raw OHLCV ──────────────────────────────────────
         ...
         ;; Squeeze: bb-width / kelt-width ratio — continuous, not boolean
         (kelt-width (if (> (abs ema20) 1e-10)
                       (/ kelt-range ema20)
                       0.0))
         (squeeze (if (> (abs kelt-width) 1e-10)
                    (/ bb-width kelt-width)
                    0.0))
