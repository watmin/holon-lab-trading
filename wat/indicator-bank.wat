...
(define (parse-minute [ts : String])
  : f64
  ; Extract minutes from "YYYY-MM-DDTHH:MM:SS" at positions 14-16.
  (+ (substring ts 14 16) 0.0))

(define (parse-hour [ts : String])
  : f64
  ; Extract hour from "YYYY-MM-DDTHH:MM:SS" at positions 11-13.
...
(define (parse-day-of-month [ts : String])
  : f64
  ; Extract day from "YYYY-MM-DDTHH:MM:SS" at positions 8-10.
  (+ (substring ts 8 10) 0.0))

(define (parse-month [ts : String])
  : f64
  ; Extract month from "YYYY-MM-DDTHH:MM:SS" at positions 5-7.
...
(define (tf-ret-val [buf : RingBuffer] [close : f64])
  ...
  (if (< (ring-len buf) 2)
    ...
    (let* ((first-val (ring-oldest buf)))
      (if (< (abs first-val) 1e-10) 0.0 (/ (- close first-val) first-val)))))
...
(define (compute-choppiness [chop-buf : RingBuffer]
  ...
  : f64
  ; CI = 100 × log(sum(ATR_14) / (highest_high - lowest_low)) / log(14).
  ...
  (if (not (ring-full? chop-buf))
    ...
    (let* ((atr-sum (ring-sum chop-buf))
           ...
           (rng (max (- hi lo) 1e-10)))
      (clamp (* 100.0 (/ (ln (/ atr-sum rng)) (ln 14.0)))
             0.0 100.0))))
...
(define (tick [bank : IndicatorBank]
  ...
  (let* (;; ── 1. Extract raw OHLCV ──────────────────────────────────────
         ...
         ;; DMI: Wilder-smoothed +DI, -DI, ADX. Period 14.
         ...
         ;; Stoch: %K = (close - low14) / (high14 - low14) × 100.
         ;; %D = SMA(3) of %K.
         ...
         ;; MFI: money-flow-ratio over 14 periods.
         (mfi (mfi-step! (:mfi bank) high low close volume))

         ;; ── 5. Volume ──────────────────────────────────────────────────

         ;; OBV: cumulative on-balance-volume.
         ;; obv-slope-12 = 12-period linear regression slope.
         (obv-slope-12 (obv-step! (:obv bank) close volume))

         ;; volume-accel: volume / SMA(volume, 20). Ratio.
         (vol-sma20 (sma-step! (:volume-sma20 bank) volume))
         (volume-accel (if (> (abs vol-sma20) 1e-10)
                         (/ volume vol-sma20)
                         1.0))

         ;; ── 6. Keltner Channels ────────────────────────────────────────
         ;; Keltner: ema20 ± 1.5 × ATR.
         ...
         ;; < 1.0 means Bollinger inside Keltner (squeeze on).
         ;; > 1.0 means Bollinger wider than Keltner (squeeze off).
         (bb-range (max (- bb-upper bb-lower) 1e-10))
         (squeeze  (/ bb-range kelt-range))

         ;; ── 7. Rate of Change ──────────────────────────────────────────
         ;; ROC-N = (close - close_N_ago) / close_N_ago. Periods: 1, 3, 6, 12.
         ;; Buffer capacity: 12.
         ...
         ;; Fraction of candles where close > prev-close. Periods: 6, 12, 24.
         ...
         ;; ── 11. Multi-timeframe ────────────────────────────────────────
         ;; 1h = aggregate 12 5-min candles. 4h = aggregate 48.
         ;; Emit close, high, low, return, body-ratio.
         ...
         ;; ── 12. Ichimoku Cloud ─────────────────────────────────────────
         ;; tenkan-sen = midpoint high/low over 9.
         ;; kijun-sen = midpoint high/low over 26.
         ;; senkou-span-a = (tenkan + kijun) / 2.
         ;; senkou-span-b = midpoint high/low over 52.
         ...
         ;; ── 13. Persistence — Hurst + autocorrelation ──────────────────
         ;; Hurst exponent: R/S analysis over close buffer(48).
         ;; Autocorrelation: lag-1 autocorrelation of close buffer(48).
         ...
         ;; Fractal-dim: Higuchi method over close buffer(30). 1.0 trending, 2.0 noisy.
         ...
         ;; ── 21. Advance bank state ─────────────────────────────────────
         (_ (set! (:prev-close bank) close))
         (_ (inc! (:count bank))))

    ;; ── 22. Assemble the enriched Candle ──────────────────────────────
    (candle
      ;; Raw
