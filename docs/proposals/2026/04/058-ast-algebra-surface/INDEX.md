...
9f. **058-036-cond** — **INSCRIPTION (2026-04-21).** `:wat::core::cond` — typed multi-way branch. Shape: `(cond -> :T ((test) body) ... (:else body))`. Typed once at the head; each test unifies with `:bool`; each body with `:T`; `:else` arm required as last — no implicit unit. Factored out of the nested-`:wat::core::if` cascade in `wat/std/hermetic.wat`'s `exit-code-prefix` (arc 012 slice 3's ceremony that `cond` eliminates). Language-core form #10, joining define / lambda / let / let* / match / if / try / struct-decl / enum-decl (plus defmacro as meta-form). Tail-position preserved via `eval_cond_tail` (arc 003's TCO discipline). wat-rs commit: `841cacc`.
